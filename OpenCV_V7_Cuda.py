...
out = cv2.VideoWriter("/home/jetson/yolo/OpenCV/OpenCV_Out_V7_cuda.avi", fourcc, fps, (resize_width, resize_height), False)
...
gpu_frame_previous = cv2.cuda_GpuMat()
...
while True:
    ...
    if ret is True:
        ...
        gpu_frame = cv2.cuda_GpuMat()  # Create a GpuMat object
        gpu_frame.upload(frame)  # Upload the frame to GPU memory
        gpu_resized = cv2.cuda.resize(gpu_frame, (resize_width, resize_height))  # Perform resizing on the GPU
        ...
        gpu_blur = cv2.cuda.createGaussianFilter(gpu_gray.type(), -1, (3, 3), 5)
        gpu_blurred = gpu_blur.apply(gpu_gray)
        times["gaussian_blur"] += time.time() - start_time

        # Perform background subtraction using frame differencing
        start_time = time.time()
        gpu_diff = cv2.cuda_GpuMat()
        
        if total_frames > 1:  # Ensure we have a previous frame to subtract from
            gpu_diff = cv2.cuda.absdiff(gpu_blurred, gpu_frame_previous)
        else:
            gpu_diff = gpu_blurred
            
        gpu_frame_previous = gpu_blurred.clone()  # Update previous frame
        times["background_subtraction"] += time.time() - start_time

        # Perform morphological operations: Erode
        start_time = time.time()
        gpu_erode = cv2.cuda.createMorphologyFilter(cv2.MORPH_ERODE, gpu_diff.type(), kernel).apply(gpu_diff)
        times["erode"] += time.time() - start_time

        # Perform dilation for multiple iterations using a loop
        start_time = time.time()
        gpu_dilate = cv2.cuda.createMorphologyFilter(cv2.MORPH_DILATE, gpu_erode.type(), kernel).apply(gpu_erode)
        times["dilate"] += time.time() - start_time

        start_time = time.time()
        gpu_close = cv2.cuda.createMorphologyFilter(cv2.MORPH_CLOSE, gpu_dilate.type(), kernel).apply(gpu_dilate)
        times["close"] += time.time() - start_time

        final_processed_frame = gpu_close.download()
        
        # Find contours in the mask
        start_time = time.time()
        contours, h = cv2.findContours(final_processed_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        out.write(final_processed_frame)
